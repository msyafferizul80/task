                            ...
                            >
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <Form.Item name="customer_name" label="Customer Name" className="col-span-2 sm:col-span-1" rules={[{ required: true, message: 'Customer name is required' }]}>
                                        <Select placeholder="Select Customer" size="large" showSearch optionFilterProp="children" disabled={(selectedTask?.status === 'DONE' && role !== 'admin' && role !== 'manager') || (role !== 'admin' && role !== 'manager')}>
                                            {customers.map(c => (
                                                <Option key={c.id} value={c.name}>{c.name}</Option>
                                            ))}
                                        </Select>
                                    ...
                                    <Form.Item name="assignee_id" label="PIC / Assignee" rules={[{ required: true, message: 'Assignee is required' }]}>
                                        <Select placeholder="Select Assignee" size="large" showSearch optionFilterProp="children" disabled={(selectedTask?.status === 'DONE' && role !== 'admin' && role !== 'manager') || (role !== 'admin' && role !== 'manager')}>
                                            {profiles.map(p => (
                                                <Option key={p.id} value={p.id}>{p.full_name}</Option>
                                            ))}
                                        </Select>
                                    ...
                                    <Form.Item name="priority_type" label="Eisenhower Priority" rules={[{ required: true, message: 'Please select a priority' }]}>
                                        <Select placeholder="Select Priority" size="large" disabled={(selectedTask?.status === 'DONE' && role !== 'admin' && role !== 'manager') || (role !== 'admin' && role !== 'manager')}>
                                ...
                                {selectedTask.created_at && (
                                 ...
                                 <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
                                     <Form.Item name="start_date" label="Start Date">
                                         <DatePicker className="w-full" size="large" showTime disabled={(selectedTask?.status === 'DONE' && role !== 'admin' && role !== 'manager') || (role !== 'admin' && role !== 'manager')} />
                                     </Form.Item>
                                     ...
                                     <Form.Item name="estimated_hours" label="Est. Hours">
                                         <InputNumber className="w-full" size="large" min={0} step={0.5} placeholder="e.g. 4.5" disabled={(selectedTask?.status === 'DONE' && role !== 'admin' && role !== 'manager') || (role !== 'admin' && role !== 'manager')} />
                                     </Form.Item>
                                 </div>
