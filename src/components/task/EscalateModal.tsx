...
export default function EscalateModal({ isOpen, onClose, task, profiles, currentUserId, currentTaskDescription, onSuccess }: EscalateModalProps) {
    ...
    return (
        <Modal
            title={<div className="font-bold text-lg mb-2 text-orange-600 flex items-center gap-2">🚩 Escalate Task</div>}
            open={isOpen}
            onCancel={() => {
                form.resetFields();
                onClose();
        ...
        >
            ...
            <Form layout="vertical" form={form} onFinish={handleEscalate}>
                ...
                >
                    <TextArea 
                        rows={4} 
                        placeholder="Contoh: Calculation siap, perlu review oleh manager..." 
                    />
                </Form.Item>
